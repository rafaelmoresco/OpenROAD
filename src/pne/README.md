...
The bias is a **feasibility-recovery mechanism**, gated on the *hard*-macro outline with a latch: it engages only while the run has not yet found a placement whose hard macros fit inside the core, and it is **permanently released for the rest of the run** as soon as any best-so-far solution is hard-feasible (`PNE-0038` reports the release point). The latch is evaluated on the best-so-far state, not the instantaneous one — the current state wanders through overflowing arrangements throughout the anneal, so an instantaneous gate stays stuck on. After release, moves revert to uniform and the outline penalty alone maintains feasibility, so wirelength optimization is not disturbed. Soft macros are virtual (no DB blockage, no PDN constraint) and are excluded from the gate, though they still act as obstacles in the slack computation.
